...
public class Breeze extends Monster {
	...
	public boolean hurt(DamageSource source, float amount) {
		if (source.getDirectEntity() instanceof Projectile) {
			this.playSound(TrialsModSounds.BREEZE_DEFLECT.get(), 1.0F, 1.0F);
			return false;
		}
		...
		if (this.getTarget() != null) {
			if (this.distanceTo(this.getTarget()) <= 4.5F && this.cd <= 0) {
				...
				if (pos != null) {
					...
					this.playSound(TrialsModSounds.BREEZE_SLIDE.get(), 1.0F, 1.0F);
					this.cd = 120;
				} else {
					this.cd = 40;
				}
	...
	public SoundEvent getAmbientSound() {
		return (this.onGround() ? TrialsModSounds.BREEZE_IDLE.get() : TrialsModSounds.BREEZE_IDLE_AIR.get());
	}

	@Override
	public SoundEvent getHurtSound(DamageSource source) {
		return TrialsModSounds.BREEZE_HURT.get();
	}

	@Override
	public SoundEvent getDeathSound() {
		return TrialsModSounds.BREEZE_DEATH.get();
	}
